...
import {
    ...
    AlertTriangle, RefreshCw, Grid, List as ListIcon, ArrowLeft
} from 'lucide-react';
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <div className="text-xs text-text-muted mb-3 flex justify-between">
                <span>{loading ? t.loadingDots : `${filteredPackages.length} ${t.packagesFound}`}</span>
            ...
            <div className="flex-1 overflow-hidden relative">
                ...
                {loading && packages.length > 0 && (
                    <div className="absolute top-0 left-0 right-0 z-10 flex justify-center pointer-events-none">
                        <div className="bg-accent/10 backdrop-blur-sm px-3 py-1 rounded-b-lg border-x border-b border-accent/20 flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-medium text-accent uppercase tracking-wider">{t.loadingDots}</span>
                        </div>
                    </div>
                )}
                ...
                <div
                    ...
                    className={`h-full overflow-y-auto custom-scrollbar pr-2 transition-opacity duration-300 ${loading && packages.length === 0 ? 'opacity-0' : 'opacity-100'}`}
                >
                    {loading && packages.length === 0 ? (
                        ...
                        <div className={viewMode === 'list' ? "space-y-2" : "grid grid-cols-2 lg:grid-cols-3 gap-3 auto-rows-fr"}>
