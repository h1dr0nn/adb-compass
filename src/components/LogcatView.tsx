...
import { ArrowLeft, FileText, Trash2, Pause, Play, Smartphone, Download, Search, X } from 'lucide-react';
...
export function LogcatView({ onBack }: LogcatViewProps) {
    ...
    useEffect(() => {
        ...
        const startStream = async () => {
            try {
                ...
                currentUnlisten = await listen<any>(`logcat-line-${selectedDevice}`, (event) => {
                    if (!isMounted) return;
                    const text = event.payload.line;
                    const level = parseLogLevel(text);
                    const newLine = { id: logCounter.current++, text, level };

                    if (pausedRef.current) {
                        logBufferRef.current.push(newLine);
                ...
                if (isMounted) {
                    ...
                    toast.error('Failed to stream Logcat');
                }
            }
        };
        ...
        return () => {
            ...
            invoke('stop_logcat_stream', { deviceId: selectedDevice }).catch(console.error);
        };

    }, [selectedDevice, maxLines]);

    const parseLogLevel = (line: string): LogLevel => {
        if (line.includes(' E ') || line.includes('/E ')) return 'E';
    ...
    const getLogLevelPriority = (level: LogLevel): number => {
        const priorities: Record<LogLevel, number> = { 'V': 0, 'D': 1, 'I': 2, 'W': 3, 'E': 4 };
    ...
    const getLogColor = (level: LogLevel) => {
        switch (level) {
            ...
            case 'W': return 'text-warning';
            case 'I': return 'text-accent';
        ...
        >
            ...
            <div className="flex items-center gap-4 mb-4">
                ...
                <div className="flex-1 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`flex h-1.5 w-1.5 rounded-full ${!paused && selectedDevice ? 'bg-success animate-pulse' : 'bg-text-muted'}`} />
                            <p className="text-xs text-text-muted">{!paused && selectedDevice ? 'Live Streaming' : 'Paused'}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all text-sm font-medium ${paused
            ...
            <div className="bg-surface-elevated border border-border rounded-xl p-3 mb-4 space-y-3 shadow-sm">
                ...
                <div className="flex items-center gap-1">
                    <span className="text-[10px] font-bold text-text-muted uppercase tracking-wider mr-2">Minimum Level:</span>
                    {logLevels.map((level) => (
                        <button
                            key={level.value}
                            onClick={() => setLogLevel(level.value)}
                            className={`px-3 py-1 rounded-md text-[11px] font-bold transition-all ${logLevel === level.value
                    ...
                    filteredLogs.length > 0 ? (
                        <div className="space-y-0.5">
                            {filteredLogs.map((line) => (
                                <div
                                    key={line.id}
                                    className={`${getLogColor(line.level)} hover:bg-white/5 px-1 rounded transition-colors group flex gap-3`}
                                >
                                    <span className="opacity-20 select-none w-8 text-right shrink-0">{line.id}</span>
            ...
            <div className="mt-2 px-2 flex justify-between items-center text-[10px] text-text-muted font-medium">
                <div className="flex items-center gap-4">
                    <span>Showing {filteredLogs.length} of {logLines.length} lines</span>
                    {paused && <span className="text-warning flex items-center gap-1"><Pause size={8} /> Stream Paused</span>}
                </div>
